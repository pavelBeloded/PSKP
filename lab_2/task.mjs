...
const server = http.createServer((req, res) => {
  ...
  if(url === '/api/name' && method === "GET") {
    res.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" });
    res.end('Белодед Павел Вячеславович');
  }

  if (url === '/xmlhttprequest' && method === 'GET') {
  ...
  if(url === '/jQuery' && method === "GET") {
    const filePath = path.join(__dirname, 'ajax.html');
